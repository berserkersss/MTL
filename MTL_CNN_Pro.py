...
if __name__ == '__main__':
    ...
    for multi in range(num_avg):
        ...
        # sample users
        dict_users = {}
        num_img_init = [1500, 1500, 1500, 1500, 1500, 1500]
        ...
        for iter in range(args.epochs):  # num of iterations
            ...
            w_locals_fl, loss_locals_fl = [], []
            for user in range(len(num_img)):

                # FL用户更新
                local = CLUpdate(args=args, dataset=dataset_train, idxs=dict_users[user])  # data select
                w_fl_temp, loss_fl = local.cltrain(net=w_fl[user].to(args.device), class_labels=num_label[user])
                w_locals_fl.append(w_fl_temp)
            ...
            # FL
            w_glob_fl = FedAvg(w_locals_fl)  # update the global model
            for user in range(len(num_img)):
                w_fl[user].load_state_dict(w_glob_fl)
        ...
        plt.xlabel('Iterations')
        plt.ylabel('Accuracy')
        plt.savefig('Figure/Accuracy_idep.png')
        ...
        ax = fig.add_subplot(111)
        for user in range(len(num_img)):
            ax.plot(acc_train_fl[user], c=colors[user], label=str(user))
        ax.legend()
        plt.xlabel('Iterations')
        plt.ylabel('Accuracy')
        ...
        for user in range(len(num_img)):
            ...
            avg += max(acc_train_fl[user]) / len(num_img)
        with open(filename, "a") as myfile:
            myfile.write(str(avg) + ',')
