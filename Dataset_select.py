...
def Mtl_sample(dict_users, num_label, num_img):
    ...
    for k in range(len(num_img)):
        while True:
            index_set, dataset_idx = Mtl_mnist_noniid(dataset_idx, num_label[k], num_img[k])
            X_train = X_all_train[index_set, :]
            y_train = y_all_train[index_set, :]
            dict_users[k] = index_set
            break

    return dict_users


def Mtl_mnist_noniid(dataset_idx, num_label, num_img):
    """
...
if __name__ == '__main__':
    ...
    # 设置用户数据集大小和标签种类
    num_img = [[1000, 800], [900, 900], [1000, 800], [900, 900],
               [1000, 800], [900, 900]]
