...
if __name__ == '__main__':
    ...
    for multi in range(num_avg):
        # sample users
        num_img = [2000, 2000, 2000, 2000, 2000, 2000]
        num_label = [[0, -1], [0, -1], [1, -1], [1, -1], [2, -1], [3, -1]]
        lambda_1 = 0.000005
        dict_users = {}
        num_img_S = [1000, 600, 600, 400, 400]
        num_label_S = [4, 4, 4, 4, 8]
        Ld_S = []
        ...
        for k in range(args.num_users):
            num_img[k].append(num_img_S[k])
            num_img[k].append(int(num_img_S[k] * args.num_users))
            num_label[k] = random.sample(label_class, num_label_S[k])
        ...
        for iter in range(args.epochs):  # num of iterations
            ...
            for user in range(len(num_img)):
                ...
                w_locals_fl.append(w_idep_temp)
            ...
            for user in range(len(num_img)):
                w_idep[user].load_state_dict(w_glob_fl)
            # select the cluster
            ...
            for user in range(len(num_img)):
                ...
                print("FL : user", user, "--Testing accuracy: {:.2f}".format(acc_test_fl2))
                #acc_train_fl_his2.append(acc_test_fl2)
